...
def calculate_sum(file):    
    ...
    return np.sum(channels), np.size(channels)
...
def calculate_derivation(file, mean):    
    ...
    if stereo:
        ...
        channels = np.array([np.abs(to_complex(data['spectrogram'][()]))])
    return np.sum(np.square(channels - mean))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', default='D:\\Data\\unmix.io\\4_training\\fft-window=1536_sample-rate=11025_channels=1-mono\\musdb18',
    ...
    with progressbar.ProgressBar(max_value=progressbar.UnknownLength) as bar:
        for file in glob.iglob(os.path.join(args.path, '**', '*.h5'), recursive=True):
    ...
    path = os.path.join(args.path, 'mean-derivation.json')
